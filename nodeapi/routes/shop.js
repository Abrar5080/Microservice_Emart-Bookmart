...
router.get("/info", async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("✅ Products counted:", productsTotal);
    } catch (err) {
    ...
    try {
      ...
      console.log("✅ Orders counted:", ordersTotal);
    } catch (err) {
    ...
    console.error("🔥 Outer error in /info route:", error);
    res.status(500).json({ success: false, message: "Failed to retrieve information" });
  }
});
